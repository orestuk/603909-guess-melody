...
const answersEl = element.querySelectorAll(`input[type="checkbox"]`);

answerButtonEl.disabled = true;
playAgainEl.addEventListener(`click`, () => renderScreen(welcomeScreenEl));
...
answerButtonEl.addEventListener(`click`, () => {
  renderScreen(resultsScreensEl[Math.floor(Math.random() * resultsScreensEl.length)]);
});
