...
import {calculatePoints, createTimer, Answer, Timer, SUCCESS_TIMEOUT} from './game-data';
...
describe(`Check calculatePoints function`, () => {
  ...
  it(`should be at least 10 right answers`, () => {
    assert.equal(calculatePoints(generateAnswers(1, 2)), -1);
    assert.equal(calculatePoints(generateAnswers(5, 1, 6)), -1);
  });

  it(`should be 10 points in case all 10 slow answers are right, and no mistakes made`, () => {
    assert.equal(calculatePoints(generateAnswers(10, 0)), 10);
  });

  it(`should be 20 points maximum if all 10 quick answers are right`, () => {
    assert.equal(calculatePoints(generateAnswers(0, 10)), 20);
  });
  ...
  it(`should be calculate right total points, slow answer +1 point, quick answer +2 points, wrong -2 points`, () => {
    ...
    assert.equal(calculatePoints(generateAnswers(slow, quick, failed)), slow * 1 + quick * 2 - failed * 2);
  });
  it(`should check if answers is array`, () => {
    assert.throw(() => calculatePoints(`45`), `Answers should be of type array`);
  });

  it(`should check if answers not undefined and not null`, () => {
    assert.throw(() => calculatePoints(undefined), `Answers could not be undefined or null`);
    assert.throw(() => calculatePoints(null), `Answers could not be undefined or null`);
  });
});

/* describe(`Check formatResults function`, () => {
  ...
  it(`should check result format in case the player has lost and he finished attempts`, () => {
    assert.equal(formatResult([], new PlayerResult(-1, 0, 40)), `У вас закончились все попытки. </br> Ничего, повезёт в следующий раз!`);
  });
  it(`should check result format in case the player won`, () => {
    ...
    assert.equal(formatResult(otherResults, oneResult), `Вы заняли 4 место из 6 игроков. </br> Это лучше, чем 33% игроков`);
    otherResults = [11, 15, 16, 18, 20, 20];
    oneResult = new PlayerResult(10, 1, 60);
    assert.equal(formatResult(otherResults, oneResult), `Вы заняли 7 место из 7 игроков. </br> Это лучше, чем 0% игроков`);
    otherResults = [11, 15, 19];
    oneResult = new PlayerResult(20, 1, 60);
    assert.equal(formatResult(otherResults, oneResult), `Вы заняли 1 место из 4 игроков. </br> Это лучше, чем 75% игроков`);
  });
}); */
